...
//                e.printStackTrace();
    ...
    private Handler mHandler = new Handler(SupplierApp.getInstance().getMainLooper()) {
        @Override
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case Constant.abnormal_Disconnection://异常断开连接
                    Utils.toast(SupplierApp.getInstance(), SupplierApp.getInstance().getString(R.string.str_disconnect));
                    break;
                case DEFAUIT_COMMAND://默认模式
                    Utils.toast(SupplierApp.getInstance(), SupplierApp.getInstance().getString(R.string.default_mode));
                    break;
                case READ_DATA:
                    ...
                    String status = SupplierApp.getInstance().getString(R.string.str_printer_conn_normal);
                    if (sendCommand == esc) {
                        //设置当前打印机模式为ESC模式
                        if (currentPrinterCommand == null) {
                            currentPrinterCommand = PrinterCommand.ESC;
                            sendStateBroadcastByBrush(CONN_STATE_CONNECTED, SupplierApp.getInstance().getString(R.string.printer_bill));
                            Utils.toast(SupplierApp.getInstance(), SupplierApp.getInstance().getString(R.string.str_escmode));
                        } else {//查询打印机状态
                            if (result == 0) {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);
                                SupplierApp.getInstance().sendBroadcast(intent);
                            } else if (result == 1) {//查询打印机实时状态
                                if ((buffer[0] & ESC_STATE_PAPER_ERR) > 0) {
                                    status += " " + SupplierApp.getInstance().getString(R.string.str_printer_out_of_paper);
                                }
                                if ((buffer[0] & ESC_STATE_COVER_OPEN) > 0) {
                                    status += " " + SupplierApp.getInstance().getString(R.string.str_printer_open_cover);
                                }
                                if ((buffer[0] & ESC_STATE_ERR_OCCURS) > 0) {
                                    status += " " + SupplierApp.getInstance().getString(R.string.str_printer_error);
                                }
                                System.out.println(SupplierApp.getInstance().getString(R.string.str_state) + status);
                                String mode = SupplierApp.getInstance().getString(R.string.str_printer_printmode_esc);
                                Utils.toast(SupplierApp.getInstance(), mode + " " + status);
                            }
                        }
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            sendStateBroadcastByBrush(CONN_STATE_CONNECTED, SupplierApp.getInstance().getString(R.string.printer_code));
                            Utils.toast(SupplierApp.getInstance(), SupplierApp.getInstance().getString(R.string.str_tscmode));
                        } else {
                            if (cnt == 1) {//查询打印机实时状态
                                ...
                                System.out.println(SupplierApp.getInstance().getString(R.string.str_state) + status);
                                String mode = SupplierApp.getInstance().getString(R.string.str_printer_printmode_tsc);
                                Utils.toast(SupplierApp.getInstance(), mode + " " + status);
                            } else {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);
                        ...
                        if (currentPrinterCommand == null) {
                            ...
                            sendStateBroadcastByBrush(CONN_STATE_CONNECTED, SupplierApp.getInstance().getString(R.string.printer_cpcl));
                            Utils.toast(SupplierApp.getInstance(), SupplierApp.getInstance().getString(R.string.str_cpclmode));
                        } else {
                            if (cnt == 1) {
                                System.out.println(SupplierApp.getInstance().getString(R.string.str_state) + status);
                                ...
                                String mode = SupplierApp.getInstance().getString(R.string.str_printer_printmode_cpcl);
                                Utils.toast(SupplierApp.getInstance(), mode + " " + status);
                            } else {//打印机状态查询
                                Intent intent = new Intent(ACTION_QUERY_PRINTER_STATE);
                                intent.putExtra(DEVICE_ID, id);
    ...
    private void sendStateBroadcast(int state) {
        ...
        intent.putExtra(DEVICE_ID, id);
        SupplierApp.getInstance().sendBroadcast(intent);//此处若报空指针错误，需要在清单文件application标签里注册此类，参考demo
    }
     ...
     * @param printName
     */
    private void sendStateBroadcastByBrush(int state, String printName) {
        Intent intent = new Intent(ACTION_CONN_STATE);
        intent.putExtra(STATE, state);
        intent.putExtra(DEVICE_ID, id);
        intent.putExtra(DEVICE_NAME, printName);
        SupplierApp.getInstance().sendBroadcast(intent);//此处若报空指针错误，需要在清单文件application标签里注册此类，参考demo
    }
